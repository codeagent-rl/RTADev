...
output a final summary in last, [MATCH] for all match result, [NOTMATCH] for not match or partilly match result.
...
In the final summary, output whether it is MATCH or NOTMATCH(warpped in [], output [MATCH] for MATCH summary and [NOTMATCH] for NOTMATCH summary).
# Attention
If the login functionality is required, there is no need to implement encrypted verification of the login password, only a simple match is required.
"""
...
output [NOTMATCH] because code can not contain pass, placeholder, can not be left for future implementation.
In the final summary, summary previous result and output whether MATCH or NOTMATCH(warpped in [], output [MATCH] for MATCH summary and [NOTMATCH] for NOTMATCH summary).
